...
python-jose
passlib
argon2-cffi [avoid bcrypt as it has limitation on 72 characters for hashing passwords]
pytest 
pytest-asyncio 
httpx
asgi_lifespan
...
Problems: Only with Uvicorn's built-in server
...
Gunicorn is a process manager.
    - A boss process that starts and supervises multiple workers.

Use Gunicorn with Uvicorn workers for production deployment

Gunicorn (Factory Manager)
│
├── Worker 1 (Uvicorn)
├── Worker 2 (Uvicorn)
├── Worker 3 (Uvicorn)
...
How request flows end to end in FastAPI with Gunicorn and Uvicorn:
...
docker build -t data-platform-fastapi .

docker image ls

docker run -p 8000:8000 data-platform-fastapi
